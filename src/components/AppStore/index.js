...
class AppStore extends Component {
  ...
  getFilteredTab = filterList => {
    const {tab} = this.state
    const filteredTabs = filterList.filter(each => each.category === tab)
    return filteredTabs
  }

  getFilteredList = () => {
    const {searchInput} = this.state
    const filteredList = appsList.filter(each =>
  ...
  render() {
    ...
    const filteredTabs = this.getFilteredTab(filteredList)
